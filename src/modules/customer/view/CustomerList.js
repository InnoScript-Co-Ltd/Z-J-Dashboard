    ...
    return (
        <>
            ...
            <div className="w-full p-3">
                <Card
                    title="Customer List"
                    subTitle="Overview of all customer list and information."
                >
                    <DataTable
                        dataKey="id"
                        size="small"
                            ...
                            return (
                                <Column
                                    ...
                                    body={(value) => {
                                        switch (col.field) {
                                            case "name":
                                                return (<ColumnNavigate url={`${paths.CUSTOMER_CREATE}/${value['id']}`} value={`${value.first_name + " " + value.last_name}`} />);
                                            case "registered_at":
                                                return <span> { new Date(value.dob).toLocaleDateString()}</span>;
                                            case "dob":
                                                return <span> { new Date(value.dob).toLocaleDateString()}</span>;
                                            case "status":
